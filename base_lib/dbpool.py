...
class DBConnection:
    ...
    def get_page_data(self, table, where=None, fields='*', other=None, isdict=True, islog={'islog': True},
                      page_num=1, page_count=20):
        """根据条件进行分页查询
        ...
        count_data = self.select_sql(table, where, "count(*) as count", other, isdict=isdict)
        all_count = int(count_data[0]["count"])
        ...
        if offset > all_count:
            ...
            data = self.select_sql(table, where, fields, other, isdict=isdict)
